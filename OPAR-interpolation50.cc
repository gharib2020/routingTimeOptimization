...
double distance(int,uint32_t,uint32_t);
...
static double warmup=100;
double precision[nWifi][nWifi]={};
...
switch (mobilityModel)
  ...
  Simulator::Schedule (Seconds (warmup), updateLifetimes);
  /////////////////////////////////////////////////////////////////////////// 
...
double precisionExpectedValue=0;
double precisionVariannce=0;
            ...
            if(precision[i][j])
               {
                 if(precision[i][j]==-1)
                    exactCount++;
                 precisionExpectedValue+=precision[i][j];
                 precisionVariannce+=precision[i][j]*precision[i][j];//to calculateE[X^2]
                 predictionCount++;
               }
          }
     }
  precisionExpectedValue=precisionExpectedValue/predictionCount;//E[X]=1/n \sum{x_i}
  precisionVariannce=precisionVariannce/predictionCount-precisionExpectedValue*precisionExpectedValue; // var(X)=E[X^2]-(E[X})^2
  
  std::cout<<"no. of predictions: "<<predictionCount<<" , E[precision]: "<<precisionExpectedValue<<" , variance(precision): "<<precisionVariannce<<" , standard deviation: "<< sqrt(precisionVariannce)<<" , no of exact predictions: "<<exactCount<<std::endl;
...
{  
  ...
  //Distance between the src and the dst in three consequent times
  double d1=distance(1,src,dst);
  double d2=distance(2,src,dst);
  double d3=distance(3,src,dst);
  
  //this parameters are to simplify the equations
  double a0=d1;
  double a1=(d2-d1)/(routCheckTime);
  double a11=(d3-d2)/(routCheckTime);
  double a2=(a11-a1)/(2*routCheckTime);
  //
...
//Bisection method to find the positive root
  ...
  while(flag)
       {
         t=tOld;
         dis1 =(a2*(2*routCheckTime*routCheckTime)+a0)+(a1+3*routCheckTime*a2)*t+a2*t*t-TrRange;
         
         t=tNew;
         dis2 =(a2*(2*routCheckTime*routCheckTime)+a0)+(a1+3*routCheckTime*a2)*t+a2*t*t-TrRange;
         ...
         if(dis1*dis2==0)
           ...
           {
             t=0.5*(tNew+tOld);
             dis3=(a2*(2*routCheckTime*routCheckTime)+a0)+(a1+3*routCheckTime*a2)*t+a2*t*t-TrRange;
             if(dis1*dis3==0)
               {
                 tNew=t;
                 flag=false;
...
double 
distance(int pos,uint32_t src,uint32_t dst)//pos represent the position of the nodes to be firs second or third
{
  double dist=0;
  int arg=0;
  switch (pos)
   ...
   case 3:
     ...
     break;
   default: 
      NS_FATAL_ERROR ("No such position:" << pos);  
   }
  dist=sqrt(pow((posMatrix[src][arg]-posMatrix[dst][arg]),2)+pow((posMatrix[src][arg+1]-posMatrix[dst][arg+1]),2)+pow((posMatrix[src][arg+2]-posMatrix[dst][arg+2]),2));//the distance of the pos(th) position 
  return dist;
}
...
checkLifetimeAccuracy()
{
 ...
 for (uint32_t i=0;i<nWifi-1;i++)
  {
    for (uint32_t j=i+1;j<nWifi;j++)
     {
       dis=distance(3,i,j);
       if (!precision[i][j] && linkLifetime[i][j] && dis>TrRange)
         {
           precision[i][j]=abs(linkLifetime[i][j]-double(Simulator::Now ().GetSeconds ())-100);
           if (precision[i][j]==0)// precise prediction
              precision[i][j]=-1;
           precision[j][i]=precision[i][j];
         }
     }
  }
 Simulator::Schedule (Seconds (1.0), checkLifetimeAccuracy);
}
