...
static int const nWifi=55;//number of UAVs
static double linkLifetime[nWifi][nWifi]={};//initilize the linkLifetime matix with all zeros
static double load[nWifi]={};//initilize the load array with all zeros  
...
static double pathLoadWeight=0.4;// 0 <=pathLoadWeight<=1
static double pathLifetimeWeight =(1-pathLoadWeight)/2;// 0 <=pathLifetimeWeight<=1
static double pathLenWeight=1-(pathLoadWeight+pathLifetimeWeight);// 0 <= pathLenWeight <= 1 
...
void removeHighestCost(double **, int *);
int routLen(int *);
...
int main (int argc, char *argv[])
{
  ...
  cmd.AddValue("pathLoadWeight","Path load weight to echo",pathLoadWeight);
  cmd.Parse (argc, argv);
...
switch (mobilityModel)
  ...
  std::cout<<"LB-OPAR, total time: "<<totalTime<<", pathLenWeight: "<<pathLenWeight<<", pathLifetimeWeight: "<<pathLifetimeWeight<<", pathLoadWeight: "<<pathLoadWeight<<", no. UAVs: "<<nWifi<<", no. Conections: "<<nSinks<<std::endl;
...
void 
removeHighestCost(double netGraph[nWifi][nWifi], int *path)
{
  int len=routLen(path);
  double highestCost=0;
  for(int i=0;i<len;i++)
     {
       if(netGraph[path[i]][path[i+1]]>highestCost)
          {
            highestCost=netGraph[path[i]][path[i+1]];
          } 
     }
  for(int i=0;i<nWifi;i++)
     {
...
Flow::findRout()
{
  ...
  for(int i=0;i<nWifi;i++)
    {
      for(int j=0;j<nWifi;j++)
         {
           if (linkLifetime[i][j]>0)
		     {
		      if(pathLifetimeWeight==0)//to skip a special case which discards some existed links
			    {
				  if(load[j]==0)
				   {
				    load[j]=0.0001;//just a small number to not be zero
				    netGraph[i][j]=pathLifetimeWeight*(1/linkLifetime[i][j])+pathLoadWeight*load[j];
				   }	
				}
			  else
			    netGraph[i][j]=pathLifetimeWeight*(1/linkLifetime[i][j])+pathLoadWeight*load[j];
  ...
  while(pathAvailable)//LB-OPAR routing
    { 
      for (int i=0; i<nWifi; i++)
         {
           path[i]=-1;
      ...
      if(path[0]==-1)
        {
      ...
      else if (objectiveValue(path)<objective)
         {
           ...
           for(int i=0;i<nWifi;i++)
               this->rout[i]=path[i];
           removeHighestCost(netGraph,path);
         }
      else
         {
           removeHighestCost(netGraph,path);
         }      
    }//end of LB-OPAR routing
