...
static double posMatrix[nWifi][1000][3]={};//each row includes sets of (x,y,z)
     ...
     for (int i=0;i<1000;i++)
       {
         for(int j=0;j<nWifi;j++)
           {
             std::cout<<posMatrix[j][i][0]<<"   "<<posMatrix[j][i][1]<<"   "<<posMatrix[j][i][2]<<"   ";
           }
         std::cout<<std::endl;
       }
  Simulator::Destroy ();
  ...
  for(int i=0;i<nWifi;i++)
     {
       ...
       posMatrix[i][traceCount][1]=currentPos.x;
       posMatrix[i][traceCount][2]=currentPos.y;
       posMatrix[i][traceCount][3]=currentPos.z;
     }
     traceCount++;
     Simulator::Schedule (Seconds (updateTime), &traceUAVpositions);
}
...
setupMobilityTrack(NodeContainer adhocNodes)
{
 ...
 Simulator::Schedule (Seconds (warmup), &traceUAVpositions);
}
